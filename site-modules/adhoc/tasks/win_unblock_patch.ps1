...
try {
    ...
    # Unhide the update using COM API to avoid prompts
    ...
    foreach ($Update in $Results.Updates) {
        if ($Update.KBArticleIDs -contains $kbnumber) {
            ...
            $result.message = "KB${kbnumber} has been successfully unblocked"
            $result.details.title = $Update.Title
            $result.details.is_hidden = $false
            break
        }
    }
    ...
    $result.status = "error"
    $result.message = "An exception occurred while unblocking KB${kbnumber}"
